...
It contains the GPU eigendecomposition code (eigen_decomp + tred2,tq12)

It also contains the C cross section code which is in the middle of being
modified to run on the GPU.  I'm starting this off by changing it minimally.
The only changes I'm making: removing any mallocs which cannot be performed on
the GPU.

Then I just wrote (untested) a function, calc_eigs_on_gpu, which calls the old
C function on the GPU and returns the eigs.  It is a GPU function which should
be run as many instances in parallel.  I am currently writting the function
gpu_driver, to call many instances of calc_eigs_on_gpu in parallel.

Actually:
besides being inneficient, all multideimnsional arrays are really difficult to
copy to and from the GPU.  Therefore, i will go through and try to convert them
to 1D arrays where an N*N array A, is addressed A[i*N + j] rather than A[i][j].
*/
...
/*I am taking the relevant code from TNT and adapting it to our problem in C.
...
array eval.  The eigenvectors will be put into evec.  This function, ofcourse,
only calculates the eigenvalues/vectors for one matrix, but it can be used as a
model to calculate many simultaneously.
...
//     }
     ...
     {
             eval[j/2].real = d[j/2];
             eval[j/2].imaginary = e[j/2];
             
             for(i = 0; i < N; i++)//row
             {
                   evec->mat[i][j].real = V[i*n + j];
                   evec->mat[i][j].imaginary = V[2*i*n + j];
             }
     }
     free(V);
     free(d);
...
{
       return (ComplexNum*) malloc(sizeof(ComplexNum)*size);
}
...
SquareMatrix *allocate_sq_mat(int numAtoms)
{
     ...
     {
           m->mat[i] = (ComplexNum *) malloc(sizeof(ComplexNum)*numAtoms);
     }
...
//I shouldn't have to modify this from the original. (not sure if I did)
__global__ void createMatrix(SquareMatrix *m, Atom *list, int numAtoms, Real q0, Real q1, Real q2, int magCellSizeA, int magCellSizeB, int magCellSizeC)
{
     int i, j, lk, l;
...
__global__ void normalize_evec(ComplexNum **evec, Atom *atom_list, int numAtoms)
{
     //This may not be the most efficient way of doing this, but I want to
     //copy the python code exactly for now.
     int i, n;
     Real norm, sigma_i;
     //Real *sigma = (Real *) malloc(sizeof(Real)*numAtoms);
...
*/     
     for(n = 0; n < numAtoms; n++)
     {
           ...
           for(i = 0; i < numAtoms; i++)
           {
                 ...
                 //norm += sigma[i]*(evec[n][i].real*evec[n][i].real + evec[n][i].imaginary*evec[n][i].imaginary);
                 sigma_i = atom_list[i].spin/fabs(atom_list[i].spin);
                 norm += sigma_i*(evec[n][i].real*evec[n][i].real + evec[n][i].imaginary*evec[n][i].imaginary);
           }
           norm = sqrt(fabs(norm));
           ...
           for(i = 0; i < numAtoms; i++)
           {
                 ...
                 evec[n][i].real = evec[n][i].real/norm;
                 evec[n][i].imaginary = evec[n][i].imaginary/norm;
           }
     }
/*     
     //free memory
...
void calc_cs(Real **mesh, int Qindex, ComplexNum **evec, Real *w, int numAtoms, Atom *atoms, Real *e, int eLen, Real ff, Real q0, Real q1, Real q2, int Na, int Nb, int Nc, Real lifeTime, Real *strfac)
{
       int i, j;
       Real tmp, y, lorentzian;
       ...
       //This is now a parameter since we can't allocate memory on the card (from the card)
       ...
       for(i = 0; i < numAtoms; i++)
       {
             ...
             {  
                   ...
                   //at (Na, Nb, Nc)
                   //remember: expon(it) = cos(t) + isin(t)
                   ...
                   //And I need to multiply by exp, which is complex (for some reason i commented that out of the python code,
                   //but I think it needs to be there).
                   tmp = atoms[j].spin/sqrt(fabs(atoms[j].spin)) * ff;
                   dum.real += tmp * (evec[j][i].real*exp.real + evec[j][i].imaginary*exp.imaginary);
                   dum.imaginary += tmp * (evec[j][i].imaginary*exp.real + evec[j][i].real*exp.imaginary);
             }
...
//Not sure what I was doing here... trying to calculate a single Q point on the GPU?
void calc_csection(Real *h, Real *k, Real *l, int numPoints, Atom *atoms, int numAtoms, Real *ff_list, Real *e, int e_len, int magCellSizeA, int magCellSizeB, int magCellSizeC, Real** mesh, Real LifeTime)
{
     int i,j;
     SquareMatrix *dev_m;
     SquareMatrix *dev_evec;
     ComplexNum *dev_eval;
     Real *dev_w;
     
     //Allocate Space on the GPU
     HANDLE_ERROR( cudaMalloc( (void**)&dev_m, n * n * sizeof(Real) ) );
     HANDLE_ERROR( cudaMalloc( (void**)&dev_evec, n * sizeof(Real) ) );
     HANDLE_ERROR( cudaMalloc( (void**)&dev_eval, n * sizeof(Real) ) );
...
//------------------------ Cross Section Code on GPU ---------------------------

//Untested 5/25/11
//These functions will need to each compute their Q-point using their blockId
//This could be made more efficient by sharing space between the square matrices
//and the V's needed for the eigendecompostion.  Also Array's or pointers should
//be converted to linear arrays to minimize memory references on the card.
//strfac could probably removed entirely... it's a temporary array
__global__ calc_cs_on_gpu(SquareMatrix **matrices, Atom *atoms, int numAtoms, Real qMin0, Real qMin1, Real qMin2, Real qStep0, Real qStep1, Real qStep2, int magCellSizeA, int magCellSizeB, int magCellSizeC, Real **Vs, Real **ds, Real **es, ComplexNum ***evecs, Real ***meshes, Real *energies, int eLen, Real *ffs, int Na, int Nb, int Nc, Real lifeTime, Real **strfacs)
{
    //this needs to be changed to include multiple threads (so we can do more
    //than 64k at once)
    int id_num = blockIdx.x;//This is the same as Qindex
    Real q0,q1,q2;
    SquareMatrix *m = matrices[id_num];
    ComplexNum **evec = evecs[id_num];
    Real **mesh = meshes[id_num];
    Real *ff = ffs[id_num];
    Real *strfac = strfacs[id_num];
    
    //eigen decomp variables
    int i,j;
    int N = m->dim;
    int n = N*2;//The TNT code uses this variable name
    Real *V = Vs[id_num]
    ...
    //calculate q point
    q0 = qMin0 + (qStep0*id_num);
    q1 = qMin1 + (qStep1*id_num);
    q2 = qMin2 + (qStep2*id_num);
    
    //create the matrix
    //m needs to be computed from the blockId and some list of matrix space that's passed in
    SquareMatrix *createMatrix(m, atoms, numAtoms, q0, q1, q2, magCellSizeA, magCellSizeB, magCellSizeC)
    
    
    //get the eigs (adapted from eigen_decomp function)
    ...
    for(i = 0; i < N; i++)
    {
     ...
     for(j = 0; j < n; j+=2)//column/value
     {
             //eigenvalues should all be real here, so I'll just use d
             //eval[j/2].real = d[j/2];
             //eval[j/2].imaginary = e[j/2];
    ...
    normalize_evec(ComplexNum **evec, atoms, numAtoms)
    
    
    //do main calculation (calc_cs)
    calc_cs(mesh, id_num, evec, d, numAtoms, atoms, energies, eLen, ff, q0, q1, q2, Na, Nb, Nc, lifeTime, strfac)
    
}

//this function will set up the computation for calc_cs_on_gpu.
void gpu_driver(Real qMin, Real qMax, int numQPoints, Real *direction, SquareMatrix **matrices, Atom *atoms, int numAtoms, int magCellSizeA, int magCellSizeB, int magCellSizeC, Real ***meshes, Real *energies, int eLen, Real *ffs, int Na, int Nb, int Nc, Real lifeTime)
{
     //Cuda variables
     SquareMatrix *dev_matrices
     
     
     
     
     //to convert to Q_points I should be able to normalize the direction
     //and then multiply by qMin and qMax to get the start and finish points
     Real qMin0, qMin1, qMin2, qMax0, qMax1, qMax2, qStep0, qStep1, qStep2;
     Real norm = direction[0]*direction[0] + direction[1]*direction[1] + direction[2]*direction[2];
     norm = sqrt(norm);//sqrt is for doubles, sqrtf is for floats
     
     qMin0 = qMin*direction[0]/norm;
     qMin1 = qMin*direction[1]/norm;
     qMin2 = qMin*direction[2]/norm;
     qMax0 = qMax*direction[0]/norm;
     qMax1 = qMax*direction[1]/norm;
     qMax2 = qMax*direction[2]/norm;
     
     Step0 = (qMax0-qMin0)/numQPoints;
     Step1 = (qMax1-qMin1)/numQPoints;
     Step2 = (qMax2-qMin2)/numQPoints;
     
     //Allocate things on this side
     //Actually everyhting allocated on the host will have to be passed in.
     //Luckilly I already have a lot of function for constructing those things
     //from python
     
     
     //Allocate things on the GPU side    
     HANDLE_ERROR( cudaMalloc( (void**)&dev_matrices, numQPoints * * sizeof(Real) ) );
     HANDLE_ERROR( cudaMalloc( (void**)&dev_evec, n * sizeof(Real) ) );
     HANDLE_ERROR( cudaMalloc( (void**)&dev_eval, n * sizeof(Real) ) );
     
     
     
     calc_cs_on_gpu<<<num,1>>>(SquareMatrix **matrices, Atom *atoms, int numAtoms, Real *qMin, Real *qStep, int magCellSizeA, int magCellSizeB, int magCellSizeC, Real **Vs, Real **ds, Real **es, ComplexNum ***evecs, Real ***meshes, Real *energies, int eLen, Real *ffs, int Na, int Nb, int Nc, Real lifeTime, Real **strfacs)
...
//This is to test the calculation of the eigenvectors on the GPU to see how 
//time vs. calculations scales.
void calc_eigs_on_gpu(SquareMatrix *m, ComplexNum *eval, SquareMatrix *evec, int N, int num)
{
     int i,j;
     int n = 2*N;//The TNT code uses this variable name
...
//     }
     ...
     tred2<<<num,1>>>(dev_V, dev_d, dev_e, n);
     
     //Diagonalize (on GPU)
     tql2<<<num,1>>>(dev_V, dev_d, dev_e, n);
     
     //Copy V,d, and e
    HANDLE_ERROR( cudaMemcpy( V, dev_V, n*n * sizeof(Real),
                              cudaMemcpyDeviceToHost ) );
    HANDLE_ERROR( cudaMemcpy( d, dev_d, n * sizeof(Real),
                              cudaMemcpyDeviceToHost ) );
    HANDLE_ERROR( cudaMemcpy( e, dev_e, n * sizeof(Real),
                              cudaMemcpyDeviceToHost ) );
...
//     printf("compex eigenvector:\n");
//     for(i = 0; i < N; i++)
//     {
//           for(j = 0; j < N; j++)
//           {
//                 //printf("mat[%d][%d] = %f + %fi\n", i, j, evec->mat[i][j].real, evec->mat[i][j].imaginary);
//                 printf("%f +%fi   ", evec->mat[i][j].real, evec->mat[i][j].imaginary);
//           }
//           printf("\n");
//     }
...
extern "C" __global__ void gpu_time_waster(float *ans_space)
{
           int i;
           float num = 0.0;
           int id_num = blockIdx.x;
           for(i = 0; i < 100000; i++)
           {
                  num += i;
           }
           ans_space[id_num] = num;
}
...
//testing to make sure that things are actually running in parallel on the card
//becuase I got wierd results when trying to run the eigendecomp function in
//parallel.  This produced a logical time vs. number of calculations graph
//stepped up every 240ish extra calculations.
//Like this:
//      ---
//   ---
//---
extern "C" void cuda_test(int num)
{
       int i, n = num;
       float *dev_ans;
       float *my_ans;
       ...
       my_ans = (float *) malloc(n*sizeof(float));
       //Allocate Space on the GPU
     HANDLE_ERROR( cudaMalloc( (void**)&dev_ans, n * sizeof(float) ) );
     
     //Copy V over to the GPU
     //HANDLE_ERROR( cudaMemcpy( dev_ans, my_ans, n*n * sizeof(Real), cudaMemcpyHostToDevice ) );
     
     //execute
     ref_ans = reference_ans();
     gpu_time_waster<<<num, 1>>>(dev_ans);
     
     
     //Copy V,d, and e
     HANDLE_ERROR( cudaMemcpy( my_ans, dev_ans, n * sizeof(float),
                              cudaMemcpyDeviceToHost ) );

     
     
    // free the memory allocated on the GPU
    HANDLE_ERROR( cudaFree( dev_ans ) );
...
extern "C" void test_func(int num)
{   
     SquareMatrix *m;
     ComplexNum *eval;
     SquareMatrix *evec;
     ...
     m = allocate_sq_mat(N);
     eval = allocateComplexArray(N);
     evec = allocate_sq_mat(N);
     
     (m->mat)[0][0].real = 2;
     (m->mat)[0][0].imaginary = 0;
     (m->mat)[0][1].real = 5;
     (m->mat)[0][1].imaginary = 2;
     (m->mat)[1][0].real = 5;
     (m->mat)[1][0].imaginary = -2;
     (m->mat)[1][1].real = 3;
     (m->mat)[1][1].imaginary = 0;

     calc_eigs_on_gpu(m, eval, evec, N, num);
