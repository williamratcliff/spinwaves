...
def run_cross_section(interactionfile, spinfile, tau_list, temperature, 
    ...
    *** Use this first to calculate cross-section before using any other methods as they all need the csection expression
    ...
    if Testing:
        ...
        #t_mats = calc_eig_mats_numerically(Hsave, h_list,k_list,l_list)
        ...
        #eigvals = generate_eigenvals(Hsave, h_list, k_list, l_list, 0.001)
        ##print "\n\nT Matrices:\n\n", t_mats, "\n\n\n\n"
        
        #print "\n\n\nHsave: ", Hsave, "\n\n\n"
        
        
        ##make all the vraiables as they are seen in McQueeny's Equations
        ##gamma_i, Sigma_i, S_i T_ni, Q, d_i
        ##gamma is the form factor
        ##sigma is the normalized spin (+1 or -1)
        ##S is the spin (I'm going to call this 'spin' to be more clear)
        ##^(here just a list of scalars becuase all spin have only one compoent along the z axis)
        ##T is the N*N matrix of eigenvectors (N eigenvectors of length N) 
        ##^This is made below and called evec
        ##Q is the (h,k,l) points which will form the x axis of our plot
        ##d is the list of position vectors in fractional coordinates (a,b,c)
        #gamma = generate_form_factors(N_atoms_uc, atom_list, hkl_interval)
        #spin = []
        #for a in atom_list:
            #spin.append(a.spin[2])
        #sigma = []
        #for s in spin:
            #sigma.append(s/np.abs(s))
        #d = []
        #for a in atom_list:
            #d.append(a.pos)
        
        #mesh = np.zeros((len(h_list), len(wt_list)))

        ##S(Q,w) = ...
        #for Q_index in range(0,len(h_list)):#loop through each Q point
            #Q = (h_list[Q_index], k_list[Q_index], l_list[Q_index])
            #t_mat = t_mats[Q_index]
            #eigs = eigvals[Q_index]
            ##normalize the matrix (copied from Mcquenies fortran code)
            ##It's my understand that N_atoms_uc should be the number of relevant cell atoms
            ##and therefore t_mat should be an N_atoms_uc*N_atoms_uc matrix, but let's make sure:
            ##My understanding was wrong!  N_atoms_uc is the atoms in a unit cell, not cutoff cell
            ##if t_mat.ndim != N_atoms_uc:
            ##    raise Exception("T_matrix dimensions != N_atoms_uc\n\nN_atoms_uc="+str(N_atoms_uc)+"\nt_mat.ndim="+str(t_mat.ndim))
            ##I'm doing a lot of indexing here which I don't need to becuase numpy would handle
            ##it for me, but I am copying McQueenys code exactly
            #evec = t_mat.copy()#my lazy way of getting an appropriately shaped matrix
            #numAtoms = np.sqrt(t_mat.size)#This should be the number of atoms that I need to loop through.
            ##numAtoms = N_atoms
            ...
            #for i in range(0, numAtoms):
                ...
                #for j in range(0, numAtoms):
                    ##sigma_j = atom_list[j].spin[2]/np.abs(atom_list[j].spin[2])
                    #normi += sigma[j] * np.abs(t_mat[j][i])**2
                #for j in range(0, numAtoms):
                    #evec[i][j] = t_mat[j][i]/np.sqrt(np.abs(normi))
            ##T_ni are the entries in the square matrix evec
            
            ##S calculation begins here
            #for w in wt_list:#loop through each w point
                ##print "Q: ", Q
                #S = (1/2.0)*(1+Q[0]**2)#*...
                ##print "S = 1/2 * (1+Q_x^2) = ", S
                #for q in range(0, numAtoms):#really this should be a vector not an int, but
                    ##each int will correspond to a vector for that atom I think
                    #for n in range(0, numAtoms):
                        #tmp = 0
                        #for i in range(0, numAtoms):
                            #tmp += gamma[Q_index]*sigma[i]*np.sqrt(spin[i])*evec[n][i]*np.exp(1j*np.dot(Q,d[i]))
                        #tmp = np.abs(tmp)**2
                    ##multiply tmp by the lorentzian
                    #y = LIFETIME_VALUE/2
                    ##Should w_n(q) be eigvals of q?  Why is there an n subscript?
                    ##I have this only multiplying for each q point becuase if I indented it and
                    ##had it count for each n too, I would just be multiplying by the same
                    ##lorentzian raised to the nth power. Come to think of it, that is exactly what
                    ##would happen to tmp above, but we are actually just recalculating it each time,
                    ##so the n loop is useless. To McQueeny's Code!
                    ##print "eigs: ", eigs
                    ##print "w: ", w
                    #lorentzian = (1/np.pi)*y/((w - eigs[q])**2+y**2)
                    #S *= lorentzian
                    ##print "S *= lorentzian = ", S
                    ##print "(j,w) = S   (",Q_index,", ",w, ") = ", S 
                    #mesh[Q_index][w] = S #add to mesh for graphing
        #print "mesh shape: ", mesh.shape
        #print "max val: ", mesh.max()
        ##plot_cross_section(h_list, wt_list, mesh*100, colorbarFlag = True)#plot with q = h
        #f = open("/home/tom/Desktop/output.txt", 'w')
        #for q in range(0,len(h_list)):
            #for w in range(0,len(wt_list)):
                #f.write(str(h_list[q]))
                #f.write(" ")
                #f.write(str(wt_list[w]))
                #f.write(" ")
                #f.write(str(mesh[q][w]))
                #f.write("\n")
        #f.close()
        
        #^That was following paper, I'm going to reogranize my code to look more like
        #McQueeny's
        # Generate Inputs
        if 1:
            ...
            #create the T_ni matricies (eigenvector matrices)
            evecs = calc_eig_mats_numerically(Hsave, h_list,k_list,l_list)
            #eigvals = calc_eigs_numerically(Hsave, h_list, k_list, l_list)#This is innefficient, eigs already calculated and thrown away in calc_eig_mats_..
            ...
            for Qindex in range(len(h_list)):
                ...
                for n in [0]:#range(numAtoms):
                    norm = 0
                    for i in range(numAtoms):
                        norm += sigma[i]*(np.abs(T[n][i])**2)
                    for i in range(numAtoms):
                        print "norm : ", norm
                        print "w sign: ", np.sign(w[n])
                        #T[n][i] = T[n][i]*np.sqrt((w[n]/np.abs(w[n]))/norm + 0j)
                        T[n][i] = T[n][i]*np.abs(np.sqrt((-np.sign(w[n]))/norm+0j))
                evec = T
                q1.append(evec[0][0])
                q2.append(evec[0][1])
                ...
                for i in [0]:#range(numAtoms):
                    dum = 0
                    for j in range(numAtoms):
                        dum += (s[j]/np.sqrt(np.abs(s[j]))) * ff * evec[j][i] * np.exp(2*np.pi*1j*(-Q[0]*basis[j][0] -Q[1]*basis[j][1] -Q[2]*basis[j][2]))
                    strfac[i] = np.abs(dum)**2
                
                sqw = np.zeros(len(e))
                for j in [0]:#range(numAtoms):
            ...
            f.close()
            #pyplot.figure()
            #pyplot.plot(Q.transpose()[0], q1, 's', markerfacecolor = 'red')
            #pyplot.plot(Q.transpose()[0], q2, 's', markerfacecolor = 'blue')
            #pyplot.show()
