    ...
    for (int xi=0; xi < XSIZE; xi++) {
        for (int yi=0; yi < YSIZE; yi++) {
            ...
            for (int zi=0; zi < nz; zi++) {
                ...
                const real rho = tex1D(tdensity,densityidx);
                Re += rho*cos(QdotR);
                Im += rho*sin(QdotR);
                densityidx++;
            }
            ...
            for (zi=0; zi < ZSIZE-(ZSIZE%4); zi+=4) {
                ...
                const real rho0 = tex1D(tdensity,densityidx);
                const real rho1 = tex1D(tdensity,densityidx+1);
                const real rho2 = tex1D(tdensity,densityidx+2);
                const real rho3 = tex1D(tdensity,densityidx+3);
...
# else  // ** This is faster but for some reason doesn't produce the correct answer.
