...
BORN_KERNEL32 = loadkernelsrc('kernel.c', precision='float32')
BORN_KERNEL64 = loadkernelsrc('kernel.c', precision='float64')
"""
...
class BornThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.precision == numpy.float32:
            ...
            #self.cudamod = cuda_texture_func(BORN_KERNEL32)
        else:
    ...
    def kernel(self):
        ...
        while True:
            ...
            print "%d of %d on %d\n"%(qxi,nqx,self.gpu),
            cuda_texture_func(nx,ny,nz,nqx,nqy,nqz,
                     cdensity,cx,cy,cz,cQx,cQy,cQz,qxi,
                     cuda.Out(result),texrefs=[texref])
