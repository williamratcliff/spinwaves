...
//std::cout<<"*"<<u<<" "<<v<<std::endl;
  ...
  INLINE Cplx& operator/=(const Cplx& r)
    {
#if 0
      /* Algorithm from /usr/local/cuda/include/cuComplex.h *
      Real s = fabs(r._real) + fabs(r._imag);
      Real sinv = 1/s;
      Real ars = _real*sinv;
      Real ais = _imag*sinv;
      Real brs = r._real*sinv;
      Real bis = r._imag*sinv;
      s = (brs*brs) + (bis*bis);
      sinv = 1/s;
      _real = ((ars*brs) + (ais*bis)) * sinv;
      _imag = ((ais*brs) - (ars*bis)) * sinv;
#else
